...
from app.exceptions.http_exceptions import producto_no_encontrado_exception, producto_existente_exception, \
...
def get_productos_by_name_and_price_range(db: Session, name: str, min_price: float, max_price: float) -> List[Producto]:
    return db.query(Producto).filter(Producto.nombre.contains(name), Producto.precio_venta >= min_price, Producto.precio_venta <= max_price).all()
